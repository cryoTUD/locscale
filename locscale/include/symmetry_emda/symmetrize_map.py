...
from locscale.include.symmetry_emda.GenerateOperators_v9_ky4 import operators_from_symbol, AngleAxis2rotatin
...
def symmetrize_map_emda(emmap_path, pg, twist=None, rise=None, n_steps=None, device=None, dtype=torch.complex128):
    """Load a map from `emmap_path` and symmetrise it; see `symmetrize_map`."""
    from locscale.include.emmer.ndimage.map_utils import load_map
    emmap, apix = load_map(emmap_path)
    return symmetrize_map(emmap, apix, pg=pg, twist=twist, rise=rise, n_steps=n_steps, device=device, dtype=dtype)
...
def symmetrize_map_helical_emda(emmap_path, twist, rise, pg="C1", n_steps=None, device=None, dtype=torch.complex128):
    from locscale.include.emmer.ndimage.map_utils import load_map
    emmap, apix = load_map(emmap_path)
    symmetry_average_map = symmetrize_map_helical(
        emmap, apix, twist, rise, pg=pg, n_steps=n_steps, device=device, dtype=dtype
    )

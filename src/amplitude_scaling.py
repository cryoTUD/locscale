...
def local_amplitude_scaling(reference_map, target_map, mask, window_size,
                            device=None, chunk=4096, dtype=torch.float32, progress_callback=None):
    ...
    *centres* (as returned by get_xyz_locs_and_indices_after_edge_cropping_and_masking); the
    ...
    corner_voxels, masked_indices, map_shape = mapops.get_xyz_locs_and_indices_after_edge_cropping_and_masking(
        mask, window_size)
    
    centres = np.asarray(corner_voxels)
    ...
    map_scaled = mapops.put_scaled_voxels_back_in_original_volume_including_padding(
        sharpened, masked_indices, map_shape)
